...
def train(model, score_func, train_pos, train_neg, x, optimizer, batch_size):
    model.train()
    score_func.train()
    ...
    for perm in DataLoader(range(train_pos.size(0)), batch_size, shuffle=True):
        ...
        edge_list = torch.tensor(edge_list).to(train_pos.device)
...
def main():
    ...
    logging.basicConfig(filename=f'Logs/GNN_train/{args.data_name}/{args.gnn_model}_{3}.log', level=logging.INFO)
    ...
    data = load_data(args.data_name, 2)
    logging.info(data)

    node_num = data.x.size(0)

    #x = data.x
    x = torch.load(f'ft_lm/embedding/{args.data_name}/bert-small_1.pt')
    x = x.to(device)
    ...
    for run in range(args.runs):
        ...
        for epoch in range(1, 1 + args.epochs):
            loss = train(model, score_func, train_pos, train_neg, x, optimizer, args.batch_size)
    ...
    logging.info(best_metric_valid_str +' ' +best_auc_valid_str)
