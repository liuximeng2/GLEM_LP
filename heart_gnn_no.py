...
def read_data(data_name, neg_mode):
    ...
    for split in ['train', 'test', 'valid']:
        ...
        if neg_mode == 'equal':
            path = 'dataset' + '/{}/{}_pos.txt'.format(data_name, split)

        elif neg_mode == 'all':
            path = 'dataset' + '/{}/allneg/{}_pos.txt'.format(data_name, split)
    ...
    for split in ['test', 'valid']:
        ...
        if neg_mode == 'equal':
            path = 'dataset' + '/{}/{}_neg.txt'.format(data_name, split)

        elif neg_mode == 'all':
            path = 'dataset' + '/{}/allneg/{}_neg.txt'.format(data_name, split)
    ...
    edge_list = [(i, i) for i in range(2)]
    edge_list = torch.tensor(edge_list).t()
    #edge_list = torch.cat((edge_list.t(), edge_list.t()[[1,0]]),dim=1)
    edge_weight_mask = torch.ones(edge_list.size(1)).to(torch.float)
    adj = SparseTensor.from_edge_index(edge_list, edge_weight_mask, [num_nodes, num_nodes])
    '''
    ...
    feature_embeddings = torch.load('dataset' + '/{}/{}'.format(data_name, 'gnn_feature'))
...
def train(model, score_func, train_pos, x, optimizer, batch_size):
    ...
    for perm in DataLoader(range(train_pos.size(0)), batch_size,
        ...
        edge_list = [(i, i) for i in range(2)]
        edge_list = torch.tensor(edge_list).to(train_pos.device).t()
        #edge_list = torch.cat((edge_list.t(), edge_list.t()[[1,0]]),dim=1)
        edge_weight_mask = torch.ones(edge_list.size(1)).to(torch.float).to(train_pos.device)
        adj = SparseTensor.from_edge_index(edge_list, edge_weight_mask, [num_nodes, num_nodes]).to(train_pos.device)
