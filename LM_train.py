...
from model import BertClassifier, BertClassifierInf, mlp_score
...
logging.basicConfig(filename=f'Logs/LM_train/{dataset}/{model_name}_es({edge_split_seed})_{label}.log', level=logging.INFO)
...
if model_name == 'bert-small':
    ...
    X = tokenizer(data.raw_texts, padding=True, truncation=True, max_length=256, return_tensors='pt')
    in_channel = 512
                ...
                if key == 'MRR':
                    ...
                    if 100 * test_hits > best_result:
                        ...
                        if 100 * test_hits > global_best_result:
                            ...
                            emb_model = BertClassifierInf(pretrained_model).to(device)
                            emb_model.load_state_dict(torch.load(PATH))
                            save_embedding(data, emb_model, emb_save_path, batch_size= 64)
