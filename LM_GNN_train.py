...
def train(model, score_func, train_pos,train_neg, x, optimizer, batch_size, args):
    model.train()
    score_func.train()
...
def main():
    ...
    parser.add_argument('--lm_model', type=str, default='sbert')
    parser.add_argument('--edge_split_seed', type=int, default=2)
    
    args = parser.parse_args()

    logging.basicConfig(filename=f'Logs/LM_GNN_train/{args.data_name}/{args.lm_model}/{args.structure}/es({args.edge_split_seed})/lr({args.lr})_mlp_layer({args.num_layers_predictor})_drop({args.dropout})_l2({args.l2}).log', level=logging.INFO)
